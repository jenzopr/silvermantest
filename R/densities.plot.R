densities.plot <- function(x, modes=1:4, mark_modes=TRUE, in_one=TRUE*(length(modes)<4)) {
  ...
  if(in_one==TRUE){
    h0=h.crit(x,modes[1])
    d=density(x,bw=h0)
    plot(d,main=paste("densities.plot"),xlim=c(-x_max,x_max),ylim=c(0,y_max),xlab="")
    for (i in 2:n){
      h0=h.crit(x,modes[i])
      d=density(x,bw=h0)
      lines(d,lty=i)
      ...
      if(mark_modes==TRUE && modes[i]==max(modes)){
        ...
        for(j in 1:modes[i]){
          ...
          lines(c(x_values[ind[j]],x_values[ind[j]]), c(y_values[ind[j]]-0.025,y_values[ind[j]]+0.025),col="red")
        }
      }
    }
  }
  #plot each densiities in one graphic
    ...
    for (i in 1:n){
      h0=h.crit(x,modes[i])
      d=density(x,bw=h0)
      plot(d,main=paste("modes:",modes[i]),ylim=c(0,y_max))
      #mark modes if option selected
      if(mark_modes==TRUE){
        ind=mode_ind(x,h0)
        x_values=d$x
   ...
   print("In the modes vector are only integer values allowed.")
  }
